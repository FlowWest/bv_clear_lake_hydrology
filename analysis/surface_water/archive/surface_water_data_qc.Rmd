...
# Surface Water Data and Groundwater Data:
...
# ACF ADOBE CRK NR FINLEY AT SODA BAY RD ----------------
# https://cdec.water.ca.gov/dynamicapp/staMeta?station_id=ACF
# TODO: what is the difference between "river stage" and "river stage aux."?
# TODO: Cameron thinks these values are off. Should explore further. 
# acf_river_stage_ft <- cdec_query(station = 'ACF', sensor_num = '1', 
#                                  dur_code = 'E', start_date = '2024-01-19',
#                                  end_date = '2024-09-06')
# 
# acf_river_stage_ft_aux <- cdec_query(station = 'ACF', sensor_num = '204', 
#                                      dur_code = 'E', start_date = '2024-01-19',
#                                      end_date = '2024-09-06')
# 
# 
# acf_river_stage_ft |> 
#   ggplot() +
#   geom_line(aes(x = datetime, y = parameter_value))
# 
# acf_river_stage_ft_aux |> 
#   ggplot() +
#   geom_line(aes(x = datetime, y = parameter_value))
```
...
```{r}
# Step 1: Add the initial depth adjustment (datum shift)
surface_water_updated <- surface_water  |> 
  mutate(
    depth_adjust = case_when(
      name == "Bell Hill Rd" & 
        datetime > as.POSIXct("2022-08-15 07:30:00", tz = "America/Los_Angeles") & 
        datetime < as.POSIXct("2022-12-07 15:30:00", tz = "America/Los_Angeles") ~ NA_real_,
      name == "Bell Hill Rd" & 
        datetime > as.POSIXct("2022-12-07 07:45:00", tz = "America/Los_Angeles") & 
        datetime < as.POSIXct("2023-06-10 15:00:00", tz = "America/Los_Angeles") ~ depth_ft - 1.9,
      TRUE ~ depth_ft
    )
  ) |> 
# Step 2: Add the second adjustment (removal of certain ranges of data)
  mutate(
    depth_adjust02 = case_when(
      name == "Bell Hill Rd" & 
        datetime > as.POSIXct("2023-04-05 08:00:00", tz = "America/Los_Angeles") & 
        datetime < as.POSIXct("2023-06-10 15:00:00") ~ NA_real_,
      name == "Bell Hill Rd" & 
        datetime >= as.POSIXct("2023-09-01 08:00:00", tz = "America/Los_Angeles") & 
        datetime < as.POSIXct("2023-09-27 09:00:00") ~ 0,
      TRUE ~ depth_adjust
    )
  )

# Step 3: Apply the vertical transformations for offsets
offsets <- tibble(
  name = c(
    "Bell Hill Rd", "Argonaut Rd", "Soda Bay Rd", 
    "Adobe Reservoir", "Highland Springs Reservoir"
  ),
  offset = c(
    mean(surface_water_updated$depth_adjust02[
      surface_water_updated$name == "Bell Hill Rd" & 
        surface_water_updated$depth_adjust02 <= 0.01], na.rm = TRUE),
    mean(surface_water_updated$depth_adjust02[
      surface_water_updated$name == "Argonaut Rd" & 
        surface_water_updated$depth_adjust02 <= 0.062], na.rm = TRUE),
    mean(surface_water_updated$depth_adjust02[
      surface_water_updated$name == "Soda Bay Rd" & 
        surface_water_updated$depth_adjust02 <= 0.087], na.rm = TRUE),
    mean(surface_water_updated$depth_adjust02[
      surface_water_updated$name == "Adobe Reservoir" & 
        surface_water_updated$depth_adjust02 <= 0.059], na.rm = TRUE),
    mean(surface_water_updated$depth_adjust02[
      surface_water_updated$name == "Highland Springs Reservoir" & 
        surface_water_updated$depth_adjust02 <= -0.086], na.rm = TRUE)
  )
)

surface_water_updated_offset <- surface_water_updated  |> 
  left_join(offsets, by = "name") |> 
  mutate(
    depth_adjust03 = case_when(
      !is.na(offset) ~ depth_adjust02 - offset,
      TRUE ~ depth_adjust02
    )
  )
...
tibble(
  Step = c("Datum Shift", 
           ...
           "Offset Correction"),
  Description = c(
    "Applied a datum shift of -1.9 ft for Bell Hill Rd data from 12/07/2022 to 06/10/2023 to align depths with known benchmarks. Removed invalid ranges prior to datum adjustment.",
    "Excluded data for Bell Hill Rd during specific periods (04/05/2023 to 06/10/2023 and 09/01/2023 to 09/27/2023) to address quality issues such as transducer removal or irregular behavior.",
...
```{r}
plot_sw_bellhill <- ggplot(data = surface_water |> filter(name == "Bell Hill Rd"), aes(x=datetime, y=depth_ft, color = "BVR EPA")) + 
  geom_line() + 
...
-   Datum shift by 1.9 feet in December 2022
...
-    Issue of the removal of the transducer...addressed by steps below that cut off Bell Hill record in April 2023
...
surface_water |> 
  ...
  ggplot(aes(x=datetime, y=depth_adjust, color=name)) + 
  geom_line() + 
  theme_light() +
  scale_color_brewer(type = 'qual', palette = 'Dark2') +
  scale_x_datetime(date_labels = "%b '%y",
...
##### Compare this to the water board data to figure out how to proceed with this data
...
-   From this we see that the datum shift makes our data match with Water Board's data at the same location for most of the winter. However, they begin to diverge in early April of 2023.
...
ggplot(data = surface_water |> filter(name == "Bell Hill Rd"), aes(x=datetime, y=depth_adjust02, color=name)) + 
  ...
  theme(axis.text.x = element_text(size = 7, angle = 45)) +
  labs(
    title = "QC data for Bell Hill after WB adjustment",
    y = "Depth (ft)",
    color = "Gage Location",
    x = "Date"
  ) 
...
-   There are issues with oscillations when creek is dry.

-   Vertically transform each dataset to get the zero flow values to roughly evenly distribute around 0 ft of depth. Then assign every value below the maximum of the oscillation to zero.
...
ggplot(data = surface_water, aes(x=datetime, y=depth_adjust03, color=name)) + 
  ...
  theme(axis.text.x = element_text(size = 7, angle = 45)) +
  labs(
    title = "Stage Data Showing Offset Correction",
    y = "Depth (ft)",
    color = "Gage Location",
    x = "Date"
  ) 
...
Quick visualization of each well to use in data memo note that I'm using the new well names for these figures, but not in the following analysis
