...
surface_water <- readRDS(here::here('data', 'surface_water', 'surface_water_data_aggregated.RDS')) |> 
  mutate(name = case_when(name == "Adobe Reservoir" ~ "Downstream of Adobe Reservoir",
                          name == "Highland Springs Reservoir" ~ "Downstream of Highland Springs Reservoir", 
...
adobe <- new_surface_water |> 
  filter(name == "Downstream of Adobe Reservoir") |> 
...
highland <- new_data_and_previous_data |> 
  filter(name == "Downstream of Highland Springs Reservoir") |> 
  ggplot(aes(datetime, y = depth_ft)) +
  geom_line(aes(color = as.factor(iteration))) +
  theme_minimal() +
  facet_wrap(~name, ncol = 2) + 
...
adoberes_offset_updated <- mean((surface_water |> filter(name == "Downstream of Adobe Reservoir"))$depth_adjust02[(surface_water |> filter(name == "Downstream of Adobe Reservoir"))$depth_adjust02 <= 0.059], na.rm = T)

highlandspringsres_offset_updated <- mean((surface_water |> filter(name == "Downstream of Highland Springs Reservoir"))$depth_adjust02[(surface_water |> filter(name == "Downstream of Highland Springs Reservoir"))$depth_adjust02 <= -0.086], na.rm = T)
```
...
surface_water <- surface_water |> 
  mutate(depth_adjust03 = case_when(
    ...
    # Following values are based on an offset that will make the lowest oscillations oscillate around zero.
    name == "Downstream of Adobe Reservoir" ~ depth_adjust02 - adoberes_offset_updated,
    name == "Downstream of Highland Springs Reservoir" ~ depth_adjust02 - highlandspringsres_offset_updated,
    T ~ depth_adjust02
  )) |> 
  mutate(
    depth_adjust03 = case_when(
      ...
      name == "Argonaut Rd" & depth_adjust03 < 0.0768 ~ 0,
      name == "Soda Bay Rd" & depth_adjust03 <= 0.0753 ~ 0,
      name == "Downstream of Adobe Reservoir" & depth_adjust03 <= 0.063 ~ 0,
      name == "Downstream of Highland Springs Reservoir" & depth_adjust03 <= 0.048 ~0,
      T ~ depth_adjust03
    )) 
...
ggsave(
  filename = here::here("analysis", "surface_water", "2024_figs", "all_sites_qcd.png"),
  width = 8,
  height = 6,
  dpi = 300
)
...
```{r}
ac4_plot <- ggplot(data = surface_water |> filter(name == "Bell Hill Rd"), 
                   aes(x = datetime, y = depth_adjust04, color = "BVR EPA")) + 
  geom_line() + 
  geom_line(data = SW_bellhill_WB, aes(x = Date, y = `Depth (ft)`, color = "Water Board")) + 
  geom_line(data = adobe_ac4_2024, aes(x = date_time, y = depth_ft, color = "Water Board")) +
  scale_color_manual(
    name = "", 
    values = c("BVR EPA" = "#1B9E77", "Water Board" = "darkblue")
  ) + 
  scale_x_datetime(
    date_labels = "%b %Y",       
    date_breaks = "1 month",     
    ...
    limits = c(
      as.POSIXct("2023-02-15 00:00:00", tz = "America/Los_Angeles"), 
      as.POSIXct("2024-12-31 00:00:00", tz = "America/Los_Angeles")
    )
  ) + 
  labs(
    x = "Date", 
    y = "Depth (ft)", 
    title = "Comparison of BVR EPA and Water Board Transducer \nMeasurements at Bell Hill Rd"
  ) +
  theme_minimal() +
  theme(
...
ggsave(
  filename = here::here("analysis", "surface_water", "2024_figs", "ac4_plot.png"),
  width = 8,
  height = 6,
  dpi = 300
)
...
ggsave(
  filename = here::here("analysis", "surface_water", "2024_figs", "soda_bay_ACF_comparison_plot.png"),
  width = 8,
  height = 6,
  dpi = 300
)
...
# STREAM GAGES 
ggplot(data = surface_water |> 
         filter(name %in% c("Soda Bay Rd", "Bell Hill Rd", "Argonaut Rd")), aes(x = datetime, y = depth_07dMA, color = name)) + 
  geom_line() + 
  theme_light() +
  scale_color_brewer(type = 'qual', palette = 'Dark2') +
  scale_x_datetime(date_labels = "%b '%y",
  ...
  theme(axis.text.x = element_text(size = 7, angle = 45)) +
  labs(
    title = "7-Day Moving Average Stage for Stream Gages",
    y = "Depth (ft)",
    color = "Gage Location",
    x = "Date"
  )
...
ggsave(
  filename = here::here("analysis", "surface_water", "2024_figs", "stream_gages_7_day_avg_plot.png"),
  width = 8,
  height = 6,
  dpi = 300
)
...
ggplot(data = surface_water |> 
         filter(name %in% c("Soda Bay Rd", "Bell Hill Rd", "Argonaut Rd")), aes(x = datetime, y = depth_01dMA, color = name)) + 
  geom_line() + 
  theme_light() +
  scale_color_brewer(type = 'qual', palette = 'Dark2') +
  scale_x_datetime(date_labels = "%b '%y",
  ...
  theme(axis.text.x = element_text(size = 7, angle = 45)) +
  labs(
    title = "1-Day (24 hour) Moving Average Stage for Stream Gages",
    y = "Depth (ft)",
    color = "Gage Location",
    x = "Date"
  )
...
ggsave(
  filename = here::here("analysis", "surface_water", "2024_figs", "stream_gages_1_day_avg_plot.png"),
  width = 8,
  height = 6,
  dpi = 300
)
...
# RESERVOIRS 
ggplot(data = surface_water |> 
         filter(name %in% c("Downstream of Highland Springs Reservoir", "Downstream of Adobe Reservoir")), aes(x = datetime, y = depth_07dMA, color = name)) + 
  geom_line() + 
  theme_light() +
  scale_color_brewer(type = 'qual', palette = 'Dark2') +
  scale_x_datetime(date_labels = "%b '%y",
  ...
  theme(axis.text.x = element_text(size = 7, angle = 45)) +
  labs(
    title = "7-Day Moving Average Stage for Gages Downstream of Reservoirs",
    y = "Depth (ft)",
    color = "Gage Location",
    x = "Date"
  )
...
ggsave(
  filename = here::here("analysis", "surface_water", "2024_figs", "reservoir_gages_7_day_avg_plot.png"),
  width = 8,
  height = 6,
  dpi = 300
)
...
ggplot(data = surface_water |> 
         filter(name %in% c("Downstream of Highland Springs Reservoir", "Downstream of Adobe Reservoir")), aes(x = datetime, y = depth_01dMA, color = name)) + 
  geom_line() + 
  theme_light() +
  scale_color_brewer(type = 'qual', palette = 'Dark2') +
  scale_x_datetime(date_labels = "%b '%y",
  ...
  theme(axis.text.x = element_text(size = 7, angle = 45)) +
  labs(
    title = "1-Day (24 hour) Moving Average Stage for Gages Downstream of Reservoirs",
    y = "Depth (ft)",
    color = "Gage Location",
    x = "Date"
  )
...
ggsave(
  filename = here::here("analysis", "surface_water", "2024_figs", "reservoir_gages_1_day_avg_plot.png"),
  width = 8,
  height = 6,
  dpi = 300
)
...
AdobeCreekTransducers_BVR_Depth_Cleaned <- surface_water |> 
  mutate(
    PT_id = case_when(
      ...
      name == "Downstream of Adobe Reservoir" ~ "AdobePT05_BVR",
      name == "Downstream of Highland Springs Reservoir" ~ "AdobePT04_BVR"
    )
  ) |> 
...
sw_wse <- surface_water |> 
  mutate(
    ...
    sensor_elev = case_when(
      ...
      name == "Downstream of Adobe Reservoir" ~ 1420.6,
      name == "Downstream of Highland Springs Reservoir" ~ 1422.9,
      T ~ 1402.281
    ),
...
ggsave(
  filename = here::here("analysis", "surface_water", "2024_figs", "water_surface_elevations_plot.png"),
  width = 8,
  height = 6,
  dpi = 300
)
...
ggsave(
  filename = here::here("analysis", "surface_water", "2024_figs", "mean_monthly_stage_plot.png"),
  width = 8,
  height = 6,
  dpi = 300
)
