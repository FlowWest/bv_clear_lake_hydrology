...
ac4_plot <- ggplot(data = surface_water |> filter(name == "Bell Hill Rd"), 
  ...
  scale_color_manual(
    name = "", 
    values = c("BVR EPA" = "#1B9E77", "Water Board" = "darkred", "Water Board 2024" = "darkblue")
  ) + 
  scale_x_datetime(
    date_labels = "%b %Y",       
    date_breaks = "1 month",     
...
# RESERVOIRS 
ggplot(data = surface_water |> 
         filter(name %in% c("Highland Springs Reservoir", "Adobe Reservoir")), aes(x = datetime, y = depth_07dMA, color = name)) + 
  geom_line() + 
  theme_light() +
  scale_color_brewer(type = 'qual', palette = 'Dark2') +
  scale_x_datetime(date_labels = "%b '%y",
  ...
  theme(axis.text.x = element_text(size = 7, angle = 45)) +
  labs(
    title = "7-Day Moving Average Stage for Reservoir Gages",
    y = "Depth (ft)",
    color = "Gage Location",
    x = "Date"
  )
...
ggplot(data = surface_water |> 
         filter(name %in% c("Highland Springs Reservoir", "Adobe Reservoir")), aes(x = datetime, y = depth_01dMA, color = name)) + 
  geom_line() + 
  theme_light() +
  scale_color_brewer(type = 'qual', palette = 'Dark2') +
  scale_x_datetime(date_labels = "%b '%y",
  ...
  theme(axis.text.x = element_text(size = 7, angle = 45)) +
  labs(
    title = "1-Day (24 hour) Moving Average Stage for Reservoir Gages",
    y = "Depth (ft)",
    color = "Gage Location",
    x = "Date"
  )
